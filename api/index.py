...
def create_employee():
    ...
    try:
        ...
        cur.execute(
            """
            INSERT INTO employees (emp_code, full_name, phone, role, monthly_salary, join_date)
            VALUES (%s, %s, %s, %s, %s, COALESCE(%s::DATE, CURRENT_DATE))
            RETURNING *
            """,
            (emp_code, full_name, phone or None, role, monthly_salary, join_date),
        )
        emp = cur.fetchone()
        conn.commit()
...
def update_employee(emp_id: int):
    ...
    try:
        ...
        if monthly_salary is not None:
            fields.append("monthly_salary = %s"); values.append(float(monthly_salary))
        if join_date:
...
def attendance_report():
    ...
    try:
        cur.execute(
            ...
            WHERE e.active = TRUE
            GROUP BY e.id, e.emp_code, e.full_name, e.role, e.monthly_salary
            ORDER BY e.full_name
            """,
            (year, mon),
        )
        ...
        working_days = sum(
            ...
            if date(year, mon, d).weekday() < 6   # Mon–Sat counted; Sun off
        )
        report = []
        for r in cur.fetchall():
            ...
            # effective days = present + half*0.5 + leave (paid leave)
            effective = present + half * 0.5 + leave
            payable   = round((salary / working_days * effective) if working_days > 0 else 0, 2)
            report.append({
        ...
        cur.execute(
            "SELECT employee_id, amount_paid, paid_at FROM salary_payments WHERE pay_month = %s",
            (month,),
        )
        paid_map = {}
        for sp in cur.fetchall():
            paid_map[sp["employee_id"]] = {
                "amount_paid": float(sp["amount_paid"]),
                "paid_at":     sp["paid_at"].isoformat() if sp["paid_at"] else None,
            }
        for row in report:
            sp = paid_map.get(row["employee_id"])
            row["salary_paid"]   = bool(sp)
            row["amount_paid"]   = sp["amount_paid"] if sp else 0
            row["salary_paid_at"] = sp["paid_at"]    if sp else None

        return jsonify({"report": report, "month": month, "working_days": working_days})
    finally:
        cur.close(); conn.close()


# ---------------------------------------------------------------------------
# Salary Payment — MARK PAID  (POST /api/salary/pay)   — Admin only
# ---------------------------------------------------------------------------
...
def mark_salary_paid():
    ...
    try:
        ...
        cur.execute(
            """
            INSERT INTO salary_payments (employee_id, pay_month, amount_paid, paid_by, notes)
            VALUES (%s, %s, %s, %s, %s)
            ...
            RETURNING *
            """,
            (employee_id, pay_month, amount_paid, paid_by, notes),
        )
        sp = _emp_to_dict(cur.fetchone())
        conn.commit()
        return jsonify({"message": "Salary payment recorded", "salary_payment": sp}), 201
    except Exception as e:
        conn.rollback()
        return jsonify({"detail": str(e)}), 400
    finally:
